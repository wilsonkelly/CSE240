...
{
	// Check if b is not full
	if(currentCount<MAX_BOOKS) {
		// Check every books ID to see if it already exists
		for (int i = 0;i < MAX_BOOKS;i++) {
			if (ID_input == b[i].getID()) {
				return 0; // Book already exists
			}
		}
		// Assign values to this new book using book.cpp
		Book newBook;
		newBook.setName(name_input);
		newBook.setAuthor(author_input);
		newBook.setYearOfPublication(yearOfPublication_input);
		newBook.setID(ID_input);
		// Could also do b[currentCount].setID(ID_input), etc.
		b[currentCount] = newBook;
		currentCount++;
		return 1; // Book added
	}
	else {
		return 0; // Book list is full;
	}
}
...
{
	// First check in case there is only 1 book
	if (n == 0) {
		// Display
		b[n].displayBook();
		n++;
	...
	// Rest of the books
	else if (n <= currentCount-1) {
		// Display
		b[n].displayBook();
		// For recursion: book list not empty yet, go to next item
		n++;
		displayBooks(n);
	}
	// While not in correct range
	else {
		// Stopping condition
		return;
	}
...
void sort()
{
	...
	for (int i = lowerBoundYear; i < currentCount; ++i) {
		for (int j = i + 1; j < currentCount; ++j) {
			//check if the year of lowerbound is greater than upperbound
...
void newestBook()
{
	char startingCharacter=1;				// Ask the user for a character
	...
	// Parse array to check if book is newest
	for (int i = 0;i < MAX_BOOKS;i++) {
		if (startingCharacter  == b[i].getName()[0]) {
			
			// Copy book into newBook
			newBook->setName( b[i].getName() );
			newBook->setAuthor( b[i].getAuthor());
			newBook->setYearOfPublication( b[i].getYearOfPublication() );
			newBook->setID( b[i].getID() );
			// Display newBook
			newBook->displayBook();
			// Delete newBook
			newBook = NULL;
