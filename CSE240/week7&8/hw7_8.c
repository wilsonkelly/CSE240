...
void addStudent(char* studentNameInput, unsigned int idInput)
{
	...
	// If list is not empty (add at the beginning)
	else {
		// Assign values
		strcpy((newStudent->student)->name, studentNameInput);
		newStudent->student->id = idInput;
...
{
	// If tempList is not empty, go through
	if (tempList != NULL) {
		// Display first position in tempList
		printf("\nStudent name : %s", tempList->student->name);
		printf("\nStudent Id : %d\n", tempList->student->id);
		// For recursion: tempList not empty yet, go to next item
...
struct student* searchStudent(char* studentNameInput)
{
	...
	while (tempList != NULL) {
		...
		if (strcmp(tempList->student->name, studentNameInput) == 0) {
			// Stop if name is found
			return tempList->student;
		}
		// If name is not found, go to next on list
		tempList = tempList->next;
	}
	//If tempList is empty, then student is original
	return NULL;
...
void addCourse(char* studentNameInput, char* courseNameInput)
{
	...
	while (strcmp(tempList->student->name, studentNameInput)) {
		// Go through
		tempList = tempList->next;
	}
	// Copy course name
	strcpy(newCourse->name, courseNameInput);
	// Add course to head of courses
...
void displayStudentCourseList(struct studentList* tempList)
{
	...
	// Chbeck to see if there are courses
	...
	// If tempList is not empty, go through
	while (tempList != NULL) {
		if (strcmp(tempList->student->courses->name, courseNameInput) == 0) {
			// Course found, display info
			printf("\nStudent name : %s", tempList->student->name);
			printf("\nStudent Id : %d\n", tempList->student->id);
			// Stop search
			return;
		}
		// Course not found, keep searching
		tempList = tempList->next;
	}
	// Since search was never stopped, no courses found 
	printf("\nNo courses found.");
}
...
void removeStudent(char* studentNameInput)
{
	...
	struct course* tempCourseList = tempList->student->courses;
	
	// If student name is found as head
	if (strcmp(list->student->name, studentNameInput)==0) {
		// Delete student
		list = list->next;
		while (tempCourseList != NULL) {
			tempCourse = tempCourseList;
			tempCourseList = tempCourseList->next;
			free(tempCourse);
		}
		free(tempList->student);
		free(tempList);
		return;
	}
	
	struct studentList* tempList2 = list->next;	// work on a copy of 'list'
	struct course* tempCourse2 = tempList2->student->courses;
	
	// If student name is not head
	while (tempList2 != NULL) {
		// Compare name again
		if (strcmp(tempList2->student->name, studentNameInput) == 0) {
			// Delete student
			tempList->next = tempList2->next;
			while (tempCourse2 != NULL) {
				tempCourse = tempCourse2;
				tempCourse2 = tempCourse2->next;
			}
			free(tempList2->student);
			free(tempList2);
			return;
		}
		tempList = tempList2;
